...
- ⬜ **HabitTask/Habit update strategy — mapper vs. native query** — deferred until frontend integration. Two approaches discussed for updating `HabitTask` (and similarly `Habit`) when relationship fields (`taskPriorityLevel`, `habitCategory`) need to change:
  1. **Mapper approach** (`HabitTaskMapper.update(request, taskPriorityLevel, habitCategory, @MappingTarget habitTask)`) — requires fetching `TaskPriorityLevel`/`HabitCategory` entities first (extra repository dependencies in the service, extra queries), but stays fully type-safe and JPQL-based.
  2. **Native SQL bulk update** (`@Query(value = "...", nativeQuery = true)` updating foreign key columns directly by ID) — avoids fetching the related entities entirely, but loses compile-time property-name safety (relies on matching raw DB column names) and surfaces foreign-key-violation errors less clearly than the current `ResourceNotFoundException` pattern.
     Decide once the actual frontend request shape/update UX is known — the choice affects what the update endpoint expects the client to send (bare IDs vs. nothing extra) and how validation errors are surfaced.

- ⬜ **Orchestration/Automation Engine** — a coordinating layer to handle initialization, calculation, and auto-generation flows across services (e.g. tying together `HabitWeek`/`HabitCalendar` generation and `PercentageDay`/`PercentageWeek` calculation into a single coherent process). Not yet designed — deliberately deferred until the core Repository/Service/Controller pass (Section 5) and the rest of this backlog are finished, so the engine is built on a stable foundation rather than mixed into ongoing layer-by-layer work. When revisiting, first resolve: (1) trigger mechanism (scheduled vs. user-registration-triggered vs. both), (2) exact scope of "auto generation" (calendar/week generation, percentage calculation, or both as one pipeline), (3) where this layer sits relative to existing services — likely a new orchestration-only class that calls into existing services rather than duplicating their logic.

- ✅ **AOP** — activated **and implemented**. Concrete use case identified: cross-cutting logging (method entry/exit, execution time, failure logging) across all service-layer methods, replacing what would otherwise be repetitive manual `log.info`/`log.error` calls in every service method. Implementation: `AopLogging` — a single `@Aspect` with an `@Around` advice on `execution(* com.mts.aadati.services.*.*(..))`, plus `AopConfig` (`@EnableAspectJAutoProxy(proxyTargetClass = true)`). Scope is intentionally limited to logging — this is not a general-purpose AOP adoption for business logic, validation, or security concerns, which stay in their existing dedicated layers (services, `@RestControllerAdvice`, Spring Security). The aspect's logging doesn't duplicate what `GlobalExceptionHandler` already logs for exception cases — the aspect covers method entry/exit and success/failure timing, the handler covers the exception details themselves.
  - First implementation draft had a critical defect: it resolved the current user via an unguarded `SecurityContextHolder`/`CustomUserDetails` cast executed *before* `joinPoint.proceed()` and outside any try/catch — meaning any unauthenticated call (`AuthenticationService.login()`/`register()` themselves, or a future `@Scheduled` job) would throw before the underlying service method ever ran, plus a malformed/mismatched pointcut and unconditional logging of raw args/return values (secret-leak risk, CWE-532). Fixed: `resolveCurrentUserId()` is now a self-contained, never-throwing helper (falls back to `"anonymous"`/`"unknown"`), the pointcut is corrected and package-verified, args/result are no longer logged, `@Order(Ordered.HIGHEST_PRECEDENCE)` added so the aspect wraps outside `@Transactional`, and the failure log now passes the exception object itself (not just its message) to preserve the stack trace. See `REFACTORING_LOG.md` → **"Fix — AOP Service-Logging Aspect Could Break the Calls It Was Meant to Only Observe"** for the full Problem/Decision/Why/Impact, and `ENGINEERING_RULES.md` §10 (AOP — Cross-Cutting Logging) for the standing rules extracted from this fix.
